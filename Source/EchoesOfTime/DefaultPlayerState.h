...
public:
    ...
    UFUNCTION(Server, Reliable)
    void ServerSetAvatarIndex(int32 NewIndex);
    ...
    // Team System
    // ============================================
    UPROPERTY(ReplicatedUsing = OnRep_TeamName, EditAnywhere, BlueprintReadWrite, Category = "Team")
    FString TeamName = "Past";

    UFUNCTION(BlueprintPure, Category = "Lobby|Team")
    FString GetTeamName() const { return TeamName; }

    UFUNCTION(Server, Reliable)
    void ServerSetTeam(const FString& NewTeam);

    UFUNCTION(BlueprintCallable, Category = "Lobby|Team")
    void SetTeamLocal(const FString& NewTeam);
