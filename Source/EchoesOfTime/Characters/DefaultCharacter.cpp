...
void ADefaultCharacter::Tick(float DeltaTime)
{
    ...
    if (IsLocallyControlled())
    {
        if (CurrentTime - LastInteractHighlightUpdateTime >= InteractHighlightUpdateInterval)
        {
            ...
            LastInteractHighlightUpdateTime = CurrentTime;
        }
    }
    ...
    // On first update, use DetectionUpdateInterval to avoid large delta
    const float ActualDelta = FMath::IsNearlyZero(LastDetectionUpdateTime) ? DetectionUpdateInterval : (CurrentTime - LastDetectionUpdateTime);
